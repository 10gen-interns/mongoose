...
Query.prototype._castUpdate = function _castUpdate (obj, overwrite) {
  ...
  while (i--) {
    ...
    if ('Object' === val.constructor.name && !overwrite) {
      ...
      // *always* return the value, even if it is an empty object. This only
      // applies to the case where the user *actually* passes an update. If
      // they send `{}` as there update doc, control flow never reaches here,
      // which is fine because we don't want to actually update anything if
      // they pass an empty document
