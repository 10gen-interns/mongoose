...
/**
 * Gets the server version, which is later used to decide certain query patterns
 *
...
Connection.prototype.getVersion = function () {
  var self = this;
  this.db.admin(function (err, admin) {
    if (err) {
      self.emit('open');
    } else {
      admin.serverStatus(function (err, info) {
        if (!err) {
          self.__mongodVersion = info.version.split('.').map(function(n){return parseInt(n, 10) });
        }
        self.emit('open');
      });
    }
